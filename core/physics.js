...
function update_game_logic(){
	...
	if(!(!window.has_focus&&d.settings.interface.pause_on_blur)){
		if(d.save.temp.room.preparation){
			f.print_to_chat(d.language.notifications.current_room(d.save.world.players[d.save.player.nickname].position.room_id));
		}
		tps_count++;
		...
		/*комнаты*/
		if(d.save.world.players[d.save.player.nickname].position.room_id=='disclaimer'){
			f.prepare(()=>{
				f.init_audio();
...
\_/ \_/ L_/ L] U U U U \_/ U U`.trim())}</div>`),
					...
					buttons:f.dict_to_buttons(d.language.rooms[d.save.world.players[d.save.player.nickname].position.room_id].buttons),
					bug_counter:0
				};
		...
		if(d.save.world.players[d.save.player.nickname].position.room_id=='character_selection'){
			f.prepare(()=>{
				...
				d.save.temp.room.data={
					...
					title:f.create_element_from_HTML(f.get_transparent_space_text(d.language.rooms[d.save.world.players[d.save.player.nickname].position.room_id].title)),
					drop_zone:f.wrap_in_frame(f.create_element_from_HTML(`<div class='drop_zone center'><div style='text-align:center;'>${f.get_transparent_space_text(d.language.rooms[d.save.world.players[d.save.player.nickname].position.room_id].drop_zone)}</div></div>`)),
					...
					Object.entries(section.options).forEach(([option_id,option])=>{
						...
						if(section_id=='interface'){
							if(option_id=='language'){
								let add_button=f.create_button_from_text(d.language.rooms[d.save.world.players[d.save.player.nickname].position.room_id].button_add);
							...
							if(option_id=='bind_to_layout'){
								...
								let create_button=(text)=>{
									...
									button.addEventListener('click',()=>{
										f.change_button_text(button,d.language.rooms[d.save.world.players[d.save.player.nickname].position.room_id].messages.input);
										setTimeout(()=>{
		...
		if(d.save.world.players[d.save.player.nickname].position.room_id=='room_editor'){
			f.prepare(()=>{
				f.set_interface_visibility(true);
				f.clear_symbols_grid();
			});
		}
		if(d.save.world.players[d.save.player.nickname].position.room_id=='intro0'){
			f.prepare(()=>{
