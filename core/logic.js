let logic_ticker=new PIXI.Ticker();
logic_ticker.add(()=>{
    document.title=`CODERROR ${window.version} TPS: ${logic_ticker.FPS.toFixed(2)} FPS: ${app.ticker.FPS.toFixed(2)} - ${window.splash}`;
    if(room=='main_menu'){
        if(preparation){
...
logic_ticker.maxFPS=60;
logic_ticker.UPDATE_PRIORITY=25;
logic_ticker.start();